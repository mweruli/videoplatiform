...
interface FeedSlideProps {
  ...
  shouldLoad: boolean
  /** Lifted to VideoFeedPlayer (not local state) so the choice carries over as the visitor scrolls to the next slide instead of resetting to muted every time — see VideoFeed.tsx. */
  muted: boolean
  onToggleLike: () => void
  onToggleFollow: () => void
  onToggleMute: () => void
 ...
 * Playback is real: a looping `<video>` plays while the slide is in view and
 * unpaused; the gradient/grain "out-of-focus footage" backdrop stays
 * underneath as a poster fallback (thumbnails aren't generated by the dev
 * video backend — see services/video.py) and while the file is loading.
 *
 * Starts muted on every slide (autoplay-without-a-gesture requires it in
 * every browser) but the speaker button in the topbar lets a visitor
 * unmute — that choice is lifted to VideoFeedPlayer so it sticks as they
 * keep scrolling, standard Shorts/Reels behaviour, rather than reverting to
 * muted on each new slide.
 */
const FeedSlide = forwardRef<HTMLDivElement, FeedSlideProps>(function FeedSlide(
  { video, index, total, liked, following, inView, shouldLoad, muted, onToggleLike, onToggleFollow, onToggleMute },
  ref,
) {
  ...
  // Keep the element's real `muted` property in sync with the lifted toggle —
  // the `muted` JSX attribute alone only sets the *initial* default, and a
  // fresh `<video>` node is created each time `shouldLoad` flips a slide into
  // range, so this re-asserts it defensively on every relevant change too.
  useEffect(() => {
    const el = videoRef.current
    if (!el) return
    el.muted = muted
