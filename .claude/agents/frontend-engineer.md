...
- **This applies especially to dashboard/internal-tool screens** (a business dashboard, an admin panel, any screen with its own navigation shell rather than just slotting into the existing public site nav) — these need real layout thought (sidebar, top bar, footer — a distinct paradigm from the consumer browsing shell), not "reuse the existing TopNav/BottomNav and improvise the rest." If asked to build one of these without a design spec, push back and ask for one explicitly rather than treating "use your judgment, stay within the design system" as license to skip it — a screen that reuses tokens/colors correctly can still fail entirely on layout, which is exactly what happened here (see docs/decisions.md for the incident).
