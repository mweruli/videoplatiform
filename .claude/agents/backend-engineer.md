...
- **Clean up after live verification.** The dockerized dev database is shared — it's what the PM actually clicks through for demos, seeded via `backend/app/db/seed_demo.py`. If you register/create test users, businesses, or products against the live stack to verify an endpoint end-to-end (not just pytest), delete them again before reporting done — same session, same tool call pattern you used to create them. Leaving "Test-<hash>"-named rows behind has happened twice already and pollutes what the PM sees; it's your job to leave the DB exactly as clean as you found it, seed data included.
